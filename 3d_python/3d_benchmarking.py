...
from ransac_pycuda_3d_level4_constant import do_ransac_3d as cuda_ransac_3d_constant
...
def plot_benchmark_const_mem(sizes, cuda_time, cuda_mem_time, const_mem_cuda_time, const_mem_cuda_mem_time, fname, xlabel, ylabel, title, logscale=True, plot_naive=False, plot_cuda_mem=False):
    ...
    if plot_cuda_mem:
        ...
        ax.plot(sizes, const_mem_cuda_mem_time, label = 'cuda_mem_transfer', marker='o')
        ax.plot(sizes, np.array(const_mem_cuda_time) - np.array(const_mem_cuda_mem_time), label = 'cuda_computation', marker='o')
    if logscale:
        ax.set_xscale('log', base=2)
    ax.legend()
    ax.grid()
...
# ----------
    ...
    # Change dataset sample size and profile the execution time to see impact of using constant memory
    if constant_mem_test:
        # Ransac parameters
        ransac_iterations = 20  # number of iterations
        ransac_threshold = 3    # threshold
        ransac_ratio = 0.6     # ratio of inliers required to assert
        ...
        const_mem_cuda_time = []
        const_mem_cuda_mem_time = []

        fname = 'ransac_3d_const_mem_cuda.png'
        xlabel='Number of data samples'
        ylabel='Execution Time (seconds)'
        title='Evaluating use of constant memory for 3D RANSAC'
        print('\n\nEvaluating the execution time with and without constant memory')
        for i, n_samples in enumerate(n_samples_all):
            ...
            print('Const memory test')
            avg_time = 0.0
            avg_mem_time = 0.0
            for i in range(num_runs):
                t2, mem_time = cuda_ransac_3d_constant(x_noise,y_noise, z_noise, ransac_iterations, ransac_threshold, ransac_ratio, n_samples, maybe_indices1, maybe_indices2, maybe_indices3)
                avg_time += t2
                avg_mem_time += mem_time
